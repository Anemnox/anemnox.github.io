...
const Terminal = (): JSX.Element => {
  ...
  const [isRunningCommand, setRunningCommand]: [boolean, Function] =
    useState(false);
  ...
  const runCommand = async (command: string, args: string[]) => {
    ...
    try {
      (commands[command as keyof typeof commands] as Command)(
        terminalInterface,
        args,
      );
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="terminal-head" onClick={toggleFullScreen}>
        Terminal
        <span className="toggleButton">{isFullScreen ? '<' : '>'}</span>
      </div>
...
const renderTerminalHistory = (history: History[]): JSX.Element[] => {
  return history.map((element: History) => {
    return (
      <div>
